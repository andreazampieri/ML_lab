...
def send(email, dataset, target_file):
    with open(target_file) as f:
        targets = f.read()

    HOST, PORT = 'lion0b.disi.unitn.it', 8888
    ...
    print('Current record on {} = {}'.format(dataset, record.strip()))
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        'dataset', choices=['ocr', 'spambase', 'tweets'],
        help='The dataset the targets belong to'
    )
    parser.add_argument(
        'target_file',
    ...
    send(args.email, args.dataset, args.target_file)
